...
-(void)viewWillAppear:(BOOL)animated{
    ...
    if ([OKProceduresManager instance].selectedProcedure != nil) {
        ...
        [OKWebServerManager getNationalDates:^(NSString *errorMsg, id responseJSON) {
            ...
            if(!errorMsg)
               [self nationalDateRequestSucceded:responseJSON];
            else
...
- (void)nationalDateRequestSucceded:(id)response
{
...
- (void)clinicalDetailsSucceded:(id)result {
    ...
    if (result && [[result objectForKey:@"status"] isEqualToString:@"true"] > 0) {
        if(isNational){
            ...
            if ([data count] == 0) {
                ...
                [OKWebServerManager getSurgeonPerformanceData:self.procedureID fromTime:self.fromDate toTime:self.toDate fromRecordNum:@"1" toRecordNum:@"1" handler:^(NSString *errorMsg, id responseJSON) {
