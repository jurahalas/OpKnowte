...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0) {
        ...
        if (self.isFilter) {
            ...
            int procedure = [[UCAppDelegate sharedObject].selectedProcedure.procedureID intValue];
            if (procedure == 10) {
                
                [UCUtility showBlockView];
                NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
                [UCWebServerHandler getOngoingClinicalDetail:[self.caseData objectForKey:@"DetailID"] withTimePointID:@"" withUserID:[defaults objectForKey:UD_USERID] withProcedureID:[UCAppDelegate sharedObject].selectedProcedure.procedureID withDelegate:self];
            }else{
